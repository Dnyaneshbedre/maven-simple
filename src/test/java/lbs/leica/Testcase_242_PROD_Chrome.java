...
public class Testcase_242_PROD_Chrome extends  ExtentReport_PROD
{
	    WebDriver driver;
	     ...
	     public void initialization()
	       {
	    	 ...
	    	 test = extent.createTest("Search All US/Global at scale ");
			...
			for (int i = 0; i < dropdown_eles.size(); i++) 
			{
				...
				for (int j = 0; j < language_radio_buttons.size(); j++) 
				{
					if (language_radio_buttons.get(j).isDisplayed()) 
					{
						...
						if(driver.findElements(By.xpath("//*[@id=\"navbar-search-selector\"]/option[1]")) != null)
					     {
					    	 System.out.println("All option is Present");
					    	 
					    	 reportLog("All option is Present");
					     }
					     else
					     {
					    	 System.out.println("All option is Absent");
					    	 
					    	 reportLog("All option is Absent");
					     }
					    ...
					    List<WebElement> No_Result = driver.findElements(By.xpath("//*[@class='tx-solr-notice nothing-found']"));
						if(No_Result.size() == 1) 
						{
							try 
							{
